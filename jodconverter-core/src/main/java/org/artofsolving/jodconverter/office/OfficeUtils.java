...
public class OfficeUtils {
    ...
    public static String getHomePath() {
        ...
        if (userDir == null) {
            userDir = System.getProperty("user.dir");     //获取当前工程路径
        }
    ...
    public synchronized static void restorePropertiesFromEnvFormat(Properties properties) {
        ...
        while (iterator.hasNext()) {
            ...
            if (value.trim().startsWith("${") && value.trim().endsWith("}")) {     //若是${}形式则从环境变量中获取
                ...
                if (envValue == null || "".equals(envValue.trim())) {        //若环境变量中不存在或为空，则剪切获取
                    value = value.substring(beginIndex + 1, endIndex);
                } else {
                    value = envValue;
                }
                properties.setProperty(key, value);                       //覆盖配置文件
            }
        }
    }
